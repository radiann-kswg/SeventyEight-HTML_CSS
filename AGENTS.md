...
> このファイルは AI エージェント（OpenAI Codex CLI 等）が本リポジトリを理解するための指示書です。
> `CLAUDE.md`（Claude Code 用）および `.github/copilot-instructions.md`（GitHub Copilot 用）と同内容を各エージェント向けに整理したものです。
> 仕様判断や運用ルールが固まった場合は、本ファイルと上記の両ファイルへ反映する前提で扱ってください。
...
- **重要な変更時は `CLAUDE.md` および `.github/copilot-instructions.md` との整合も確認してください。**
